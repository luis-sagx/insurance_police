...
public class PropietarioService {
    ...
    public PropietarioDTO buscarPorNombre(String nombreCompleto) {
        ...
        // Buscar todos los propietarios con ese nombre y tomar el más reciente
        List<Propietario> propietarios = propietarioRepository
                .findByNombreAndApellidoOrderByIdDesc(nombre, apellido);
        
        if (propietarios.isEmpty()) {
            throw new RuntimeException("Propietario no encontrado con nombre: " + nombreCompleto);
        }
        
        // Devolver el más reciente (primero en la lista)
        return convertirADTO(propietarios.get(0));
    }
    
    public PropietarioDTO buscarOCrear(String nombreCompleto, int edad) {
        String[] partes = nombreCompleto.split(" ");
        String nombre = partes[0];
        String apellido = partes.length > 1 ? partes[1] : "";

        // Buscar si ya existe un propietario con ese nombre
        List<Propietario> propietarios = propietarioRepository
                .findByNombreAndApellidoOrderByIdDesc(nombre, apellido);
        
        if (!propietarios.isEmpty()) {
            // Si existe, devolver el más reciente
            return convertirADTO(propietarios.get(0));
        }
        
        // Si no existe, crear uno nuevo
        PropietarioDTO nuevoDTO = new PropietarioDTO();
        nuevoDTO.setNombreCompleto(nombreCompleto);
        nuevoDTO.setEdad(edad);
        return crear(nuevoDTO);
    }
